...
def gather_visuals(cfg: dict, script: dict, out_dir: Path, duration: float,
    ...
    # NOTE: AI image generators (fal/replicate) are EXCLUDED from the fallback
    # chain. They can generate people/figures that don't match the theme/script,
    # so we only use real stock footage that is actually related to the topic.
    chain_map = {"auto": cfg["visuals"].get("fallback_chain", ["pexels", "scraper"])}
    # Filter out AI providers from the chain — only use real B-roll sources
    ...
    used_urls: set = set()  # avoid reusing the same scraper URL in one video
    _qcache: dict[str, Path] = {}  # query -> gathered file (stretched beats reuse it)
    _quses: dict[str, int] = {}  # query -> times already used (for variety)
    paths: list[Path] = []
    for i, q in enumerate(queries):
        dest = clips_dir / f"{i:02d}"
        if q and q in _qcache and _qcache[q].exists():
            # same visual_cue stretched over several beats: copy instead of
            # re-searching/re-downloading (turns 15 network hits into ~4)
            src = _qcache[q]
            clip = dest.with_suffix(src.suffix)
            shutil.copyfile(src, clip)
            src_json = src.with_suffix(".json")
            if src_json.exists():
                shutil.copyfile(src_json, dest.with_suffix(".json"))
            paths.append(clip)
            log(f"visual {i} ('{q[:30]}') query cache hit", "info")
            continue
        ...
        for p in chain:
            try:
                if p == "pexels":
                    clip = _pexels_video(q, dest, cfg, used_ids)
                elif p == "scraper":
                    clip = _scrape_visual(q, dest, cfg, used_urls, topic_words,
                                          prefer_videos, source_priority,
                                          _quses.get(q, 0))
                elif p in ("fal", "replicate"):
                    clip = _ai_image(q, dest, cfg, p)
                else:
                    raise ValueError(f"Unknown visuals.provider: {p}")
                _validate_clip(clip, cfg, topic_words)
                break
            except Exception as e:  # one bad clip shouldn't kill the run
                log(f"visual {i} ('{q[:30]}') provider={p} failed: {e}", "warn")
                clip = None
                continue
        if clip is None:
            if paths:
                log(f"visual {i} ('{q[:30]}') reusing previous", "warn")
                clip = paths[-1]
            else:
                continue
        else:
            final = dest.with_suffix(clip.suffix)
            if clip != final and clip.exists():
                if final.exists():
                    final.unlink()
                clip.rename(final)
            clip = final
...
def _download_ytdlp(page_url: str, dest: Path, cfg: dict) -> Path:
    max_s = int(cfg["visuals"].get("scraper_max_seconds", 8))
    ...
    try:
        ...
        pass
    base = ["--no-playlist",
            "--match-filter", "duration > 30",  # skip very short videos (<30s)
            "--format", "mp4[height<=1080]/mp4/best[ext=mp4]/best",
            "--download-sections", f"*00:00-00:0{max_s}",
            "--force-keyframes-at-cuts", "--socket-timeout", "20",
            "-o", str(out), "--retries", "2", "--no-warnings", page_url]
    cmd = ["yt-dlp", *base] if shutil.which("yt-dlp") else ["python3", "-m", "yt_dlp", *base]
    proc = subprocess.run(cmd, capture_output=True, text=True,
                          timeout=int(cfg["visuals"].get("scraper_timeout", 90)))
    if proc.returncode != 0 or not out.exists():
        raise RuntimeError(f"yt-dlp download failed: {(proc.stderr or proc.stdout)[-500:]}")
    return out
...
def _search_youtube(query: str, n: int, cfg: dict) -> list[Candidate]:
    n = max(1, min(n, 5))
    cmd = ["yt-dlp", "--no-playlist", "--flat-playlist",
           "--print", "%(id)s\t%(title)s\t%(duration)s",
           f"ytsearch{n}:{query}"]
    if not shutil.which("yt-dlp"):
        cmd = ["python3", "-m", "yt_dlp", *cmd[1:]]
    proc = subprocess.run(cmd, capture_output=True, text=True,
